...
def configure(env):
	...
	env.Append(CPPPATH=['#platform/rpi_x11'])
	if (env["use_llvm"]=="yes"):
	...
	env.Append(CPPFLAGS=['-DX11_ENABLED','-DUNIX_ENABLED','-DGLES2_ENABLED','-DNEON','-I/opt/vc/include/','-I/opt/vc/include/interface/vcos/pthreads/','-I/opt/vc/include/interface/vmcs_host/linux/','-fpermissive'])
	env.Append(LIBS=['c','m','stdc++','GLESv2', 'EGL', 'pthread','asound','z','Xau','Xdmcp','Xrender','Xfixes','Xext','bcm_host']) #TODO detect linux/BSD!
	env.Append(LINKFLAGS=['-L/opt/vc/lib'])
	#env.Append(CPPFLAGS=['-DMPC_FIXED_POINT']) ,'IMGegl','srv_um'
